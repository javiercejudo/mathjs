...
exports.getInstance = function () {
  return require('arbitrary-precision')(require('decimaljs-adapter'));
};
...
exports.e = memoize(function (precision) {
  var Big = exports.getInstance();

  Big.setPrecision(precision);

  return new Big(new Big('1').val().exp().toString());
});
...
exports.phi = memoize(function (precision) {
  ...
  return new Big('1').plus(new Big(new Big('5').val().sqrt().toString())).div(new Big('2'));
});
...
exports.pi = memoize(function (precision) {
  ...
  Big.setPrecision(precision + 4);

  var pi4th = new Big('4').times(arctan_taylor(new Big('1').div(new Big('5'))))
      .minus(arctan_taylor(new Big('1').div(new Big('239'))));
...
exports.cos_sin_sec_csc = function (x, Big, mode, reciprocal) {
  if (x.val().isNaN() || !x.val().isFinite()) {
    return new Big(NaN);
  }
  var precision = Big.getPrecision();

  // Avoid changing the original value
  var y = new Big(x.toString()).val();
  ...
  y = y[0];
  if (mode) {
    ret = cos_sin_taylor(y.div(new Big('3125')), mode);
    Big.setPrecision(Math.min(precPlusGuardDigits, precision + 15));
  ...
  Big.setPrecision(precision);
  return new Big(ret.val().toDP(precision - 1).toString());
};
...
function arctan_taylor(x) {
  ...
  for (var k = 3; !y.val().equals(yPrev); k += 2) {
    num = num.times(x2);
    ...
    if (add) {
      y = y.plus(num.div(new BigNumber(k.toString())));
    } else {
      y = y.minus(num.div(new BigNumber(k.toString())));
    }
  }
...
function cos_sin_taylor(x, mode) {
  ...
  for (var k = mode; !y.val().equals(yPrev); k += 2) {
    num = num.times(x2);
    den = den.times(k+1).times(k+2);

    yPrev = y;
    add = !add;
    y = (add) ? y.plus(num.div(new Big(den.toString()))) : y.minus(num.div(new Big(den.toString())));
  }
...
function reduceToPeriod(x, precision, mode) {
  ...
  if (x.val().abs().lte(pi.val().toDP(x.val().dp()))) {
    return [x, false];
  }

  var Big = exports.getInstance();
  // Catch if input is tau multiple using pi's precision
  if (x.div(new Big(pi.val().toDP(x.val().dp()).toString())).val().toNumber() % 2 == 0) {
    return [new Big(mode ^ 1), true];
  }

  var y = new Big(x.val().mod(tau).toString());

  // Catch if tau multiple with tau's precision
  if (y.val().toDP(x.val().dp(), 1).isZero()) {
    return [new Big(mode ^ 1), true];
  }
